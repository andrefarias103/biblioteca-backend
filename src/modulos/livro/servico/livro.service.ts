...
export class LivroService {
  constructor(
    ...
    @Inject(AUTORPORLIVRO_REPOSITORIO)
    private readonly autorPorLivroRepositorio: IAutorPorLivroRepositorio,
  ) {}
  ...
  async cadastrar(dadosLivro: CadastraLivroDto): Promise<ListaLivroDto> {
    ...
    if (!autorPorLivros) {
      throw new HttpException(
        `Não foi informado nenhum autor`,
        HttpStatus.NOT_FOUND,
      );
    }

    const verificacaoAutores: Boolean =
      await this.autorRepositorio.checaSeTodosIdsExistem(autorPorLivros);
  ...
  async buscaLivroPorAutor(id: string): Promise<ListaLivroDto[] | null> {
    const livrosId: string[] =
      await this.autorPorLivroRepositorio.buscarLivroPorAutor(id);

    if (!livrosId) {
      throw new HttpException(
        `Não foi informado nenhum livro para esse autor`,
        HttpStatus.NOT_FOUND,
      );
    }

    const livrosDoAutor = await this.livroRepositorio.ObtemlistarPorId(
      livrosId,
    );
